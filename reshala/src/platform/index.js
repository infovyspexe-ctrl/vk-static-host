...
// VK Mini Apps ВСЕГДА добавляет launch-параметры в URL (vk_app_id, vk_user_id, ...) —
// надёжный признак площадки. На VK сейв идёт в облако VK (VKWebAppStorageGet/Set) и
// синхронизируется между устройствами/версиями (требование VK 2.3.8), а не через GamePush.
function isVkLaunch() {
...
// ─────────────────────────────────────────────────────────────────────────────
// ВЫБОР ПЛОЩАДКИ. VK Mini Apps — по launch-параметрам в URL (площадка сама их добавляет).
// Иначе GamePush подхватывается автоматически, как только заполнены PROJECT_ID/PUBLIC_TOKEN,
// — иначе напрямую Яндекс.
