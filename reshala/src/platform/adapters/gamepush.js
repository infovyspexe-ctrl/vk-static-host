...
//
// СТАТУС: ПОДКЛЮЧЁН для reshala (projectId 29354). Слой выбирает этот адаптер, когда
// заполнены PROJECT_ID/PUBLIC_TOKEN (см. ../index.js). Осталось в панели GamePush:
// объявить строковое поле `save`, числовое поле лидерборда `days`; в консоли Яндекса —
// внести gamepush.com в CSP.
//
...
const PROJECT_ID = '29354';                              // из панели gamepush.com → проект → Настройки
const PUBLIC_TOKEN = 'pV9snpWfnCdHqBz10jm4zrKHnCOZ5rah'; // там же
...
export const GamePushAdapter = {
  ...
  async init() {
    ...
    this.gp = await new Promise((resolve) => {
      // Гарантия ОДНОГО исхода: и onGPInit, и onerror, и таймаут зовут done().
      // Без этого медленный/висящий gamepush.com оставляет await навечно, и игра
      // не стартует — чёрный экран. onerror ловит только ОКОНЧАТЕЛЬНЫЙ провал
      // загрузки (net::ERR_*), а он на таймауте приходит через минуты. Поэтому
      // свой предохранитель: не дождались SDK за TIMEOUT — идём в локальный режим.
      let settled = false;
      const done = (gp) => { if (settled) return; settled = true; clearTimeout(t); resolve(gp); };
      // 5 с: когда хост одобрен, SDK поднимается за ~секунду, так что на живых игроков
      // таймаут не влияет. Срабатывает, только если gamepush.com недоступен (заблокирован
      // CSP до одобрения или сеть режет) — тогда игра быстро уходит в локальный режим, а не
      // висит. Требование Яндекса для заявки CSP: игра запускается БЕЗ доступа к хостам.
      const TIMEOUT = 5000;
      const t = setTimeout(() => {
        console.warn('[platform:gamepush] SDK не ответил за ' + TIMEOUT + ' мс — локальный режим');
        done(null);
      }, TIMEOUT);
      window.onGPInit = (gp) => done(gp);
      ...
      s.onerror = () => { console.warn('[platform:gamepush] SDK не загрузился'); done(null); };
  ...
  leaderboard: {
    ...
    async getPlayerEntry(a, name) {
      if (!a.available) return null;
      // fetchPlayerRating — место игрока в таблице по полю `name` (open* открывает нативный
      // оверлей, для данных не годится).
      try { return await a.gp.leaderboard.fetchPlayerRating({ orderBy: name, includeFields: [name] }); }
      catch (e) { return null; }
    },

    async getTop(a, name, quantityTop = 10) {
      if (!a.available) return [];
      // fetch отдаёт данные таблицы; open* — нативный UI-оверлей (данные не возвращает).
      // Таблица ранжируется по объявленному числовому полю `name` (у нас — `days`).
      try {
        const res = await a.gp.leaderboard.fetch({ orderBy: name, order: 'DESC', limit: quantityTop, includeFields: [name] });
        return (res && (res.players || res.records)) || (Array.isArray(res) ? res : []);
      } catch (e) { return []; }
    },
  },
};
