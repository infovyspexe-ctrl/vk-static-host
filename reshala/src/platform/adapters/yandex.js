...
// Подгрузка SDK Яндекса своим скриптом. index.html его тегом НЕ подключает (игра идёт
// через GamePush), поэтому для прямого режима — и для гибрид-подстраховки, когда GamePush
// не поднялся, — адаптер грузит /sdk.js сам. Внутри iframe площадки /sdk.js отдаёт Яндекс;
// вне площадки (локалка, файл) скрипт не находится — тогда onerror/таймаут, и режим без SDK.
