...
// Вызвать один раз после инициализации SDK (PreloadScene). Без SDK — no-op,
// дальше работает обычный localStorage.
export async function initSafeStorage(ysdk) {
  try {
    if (ysdk && ysdk.getStorage) safe = await ysdk.getStorage();
  } catch (e) {
    safe = null; // не смогли — не страшно, ниже фолбэк
  }
}
