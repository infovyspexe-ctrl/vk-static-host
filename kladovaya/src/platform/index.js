...
// у vk-bridge не годятся: тот же признак даёт обычный iframe Яндекса.
function isVkLaunch() {
...
// ─────────────────────────────────────────────────────────────────────────────
// ВЫБОР ПЛОЩАДКИ. Меняется только эта строка.
// VK Mini Apps определяется по launch-параметрам в URL. Иначе GamePush подхватывается
// автоматически, как только в его адаптере заполнены PROJECT_ID и PUBLIC_TOKEN, —
// иначе работаем напрямую с Яндексом.
const ADAPTER = isVkLaunch() ? VkAdapter : (GamePushAdapter.configured() ? GamePushAdapter : YandexAdapter);
// ─────────────────────────────────────────────────────────────────────────────
...
export const GAME_ID = 'kladovaya';
if (GAME_ID === 'game') {
