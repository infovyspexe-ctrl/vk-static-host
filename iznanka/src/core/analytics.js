import { isGameSmile } from '../platform/adapters/gamesmile.js';
...
export const Analytics = {
  ...
  init(id, prefix) {
    ...
    // На нашем собственном сайте чужих счётчиков быть не должно. Метрика этой игры
    // заведена под её страницу на Яндекс Играх: на gamesmile.ru она собирала бы наших
    // игроков в чужой счётчик и тянула бы сторонний скрипт в игру, встроенную к нам
    // в iframe. Адаптер gamesmile существует ровно затем, чтобы внешнего SDK не было, —
    // аналитика не должна пробивать это правило в обход. Своя приедет с мини-API сайта.
    if (isGameSmile()) {
      console.warn('[Analytics] свой сайт — события только в консоль');
      return;
    }
