...
export function setupLifecycle(game) {
  ...
  // СТОРОЖ ЗАВИСШЕЙ ПАУЗЫ ОТ BLUR. Событие 'focus' на window — единственный природный
  // сигнал снять эту паузу, а у ad-флоу (platform/index.js) на свой источник паузы уже
  // есть 45-секундный сторож. У blur/focus сторожа не было вовсе — и живой прогон на
  // РЕАЛЬНОМ драфте (26.08) поймал именно эту дыру: вкладка видна, document.hasFocus()
  // реально true, а 'focus' на window просто не пришёл (вложенный iframe площадки) —
  // paused остаётся true НАВСЕГДА, все сцены из pausedScenes по-настоящему стоят на
  // паузе у Phaser, экран выглядит нормально, но ни одна кнопка не отвечает, ни единого
  // исключения в консоли. Ровно тот класс «зависшего» экрана, что ловили на разных
  // сценах всю неделю. Подстраховка — не ждать событие, а раз в секунду сверять
  // РЕАЛЬНОЕ состояние фокуса напрямую; maybeResume() и так безопасно вызывать лишний
  // раз (canResume() отсекает скрытую вкладку и активную рекламу).
  setInterval(() => { if (paused && !document.hidden && document.hasFocus()) maybeResume(); }, 1000);
