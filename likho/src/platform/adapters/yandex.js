...
function loadYandexSdk(timeoutMs = 5000) {
  return new Promise((resolve) => {
    ...
    // странице. Ждём тот, что уже в полёте.
...
// Прямой сигнал «игра загрузилась» Яндексу — НЕЗАВИСИМО от выбранного адаптера.
// Консоль Яндекса не пускает на модерацию без LoadingAPI.ready() («игра ни разу не
// запускалась»), и это фактор ранжирования. Через GamePush сигнал идёт вызовом
// gp.gameStart(), но маппинг в LoadingAPI.ready НЕ подтверждён и на деле не сработал
// (отказ отправки на модерацию «Решала» 2026-08-03). Поэтому на Яндексе всегда зовём
// ready напрямую. YaGames внутри Яндекса уже загружен — нашим адаптером ИЛИ самим
// GamePush (он тоже поднимает Yandex SDK). Вне Яндекса (VK, локалка, файл) YaGames нет
// — тихо выходим, ничего не грузим.
// Сигнал запуска шлём РОВНО ОДИН РАЗ. Путей два — прямой signalYandexReady() и адаптерный
// ready(), и Platform.ready() зовёт оба. Без флага SDK ругается «Don't call
// features.LoadingAPI.ready() more than one time», а повторный сигнал портит замер времени
// загрузки, по которому Яндекс ранжирует игру. Флаг модульный: адаптер GamePush его не
// трогает, поэтому в режиме GamePush прямой сигнал по-прежнему уходит.
let readySent = false;

export async function signalYandexReady() {
  try {
    if (readySent) return;
    if (typeof YaGames === 'undefined') return;
    const sdk = window.ysdk || await YaGames.init();
    window.ysdk = sdk;
    sdk.features.LoadingAPI.ready();
    readySent = true;
    console.log('[platform:yandex] LoadingAPI.ready() — прямой сигнал запуска');
  } catch (e) { console.warn('[platform:yandex] прямой ready не удался', e); }
}
...
export const YandexAdapter = {
  ...
  // Проверка «подключился ли сам скрипт SDK» здесь НЕ годится (первая версия этой правки,
  // ошибка найдена на «Изнанке» 04.08 живым плейтестом): тег <script src="…yandex.ru/
  // games/sdk/v2"> публично доступен и загрузится даже при обычном прямом открытии на
  // dev-сервере вне какой-либо площадки — YaGames там определяется, просто сам init()
  // зависает без ответа родителя. iframe — единственный технически проверяемый признак,
  // что мы вообще МОГЛИ БЫ быть на площадке. Разница важна для ads ниже: если мы внутри
  // iframe, но init не успел (медленная сеть ВНУТРИ настоящего Яндекса) — подставлять
  // фейковую рекламу/награду вместо настоящей нельзя (запрет 1.16 на имитацию рекламных
  // блоков). Заглушка законна только вне какого-либо iframe — то есть игра точно не на
  // площадке.
  ...
  async init() {
    ...
    // «No parent to post message», за которыми не видно настоящих ошибок игры.
    ...
    try {
      ...
      // (белый экран). Найдено на «Решале» (2026-07-17), перенесено в шаблон 2026-07-19.
      //
      // 12 секунд и ДВЕ попытки, а не 3 секунды и одна. Прежние 3 с калечили сессию тем, у
      // кого медленный мобильный интернет ВНУТРИ настоящей площадки: не уложился — и до
      // перезагрузки нет ни рекламы (доход ноль), ни облачных сейвов, ни лидерборда, причём
      // молча. Находка красной команды 12.08; та же величина и та же схема уже применены в
      // «Орбитальном рубеже», прошедшем модерацию.
      const initOnce = (ms) => Promise.race([
        YaGames.init(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('init timeout')), ms)),
      ]);
      try {
        this.ysdk = await initOnce(12000);
      } catch (first) {
        console.warn('[platform:yandex] init не успел за 12с — вторая попытка', first);
        this.ysdk = await initOnce(12000);
      }
      this.available = true;
  ...
  ready() {
    if (readySent || !this.available) return;   // сигнал шлётся ровно один раз, см. readySent выше
    try { this.ysdk.features.LoadingAPI.ready(); readySent = true; } catch (e) {}
  },
  ...
  ads: {
    ...
    // У Яндекса нет API «предзагружен ли ролик» — отвечаем true, иначе кнопка награды
    // не появилась бы никогда. Отсутствие инвентаря отработает onClose(false) при показе.
    hasRewarded() { return true; },
  ...
  // Please, use ysdk.leaderboards» — находка на «Мойке Мечты» 05.08 (реальный ERROR, не
  // косметика, ловит B7). Проверено документацией Яндекса перед заменой
  // (yandex.ru/dev/games/doc/dg/sdk/sdk-leaderboard).
  leaderboard: {
