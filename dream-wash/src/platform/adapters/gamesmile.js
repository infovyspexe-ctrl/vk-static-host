...
export const GameSmileAdapter = {
  ...
  // ТАБЛИЦА РЕКОРДОВ площадки. Игра зовёт то же самое, что и на Яндексе, —
  // Platform.leaderboard.setScore(имя, число), — а куда это уедет, решает
  // адаптер. Игровой код про площадку по-прежнему ничего не знает.
  //
  // Имя таблицы (name) мы НЕ используем: у игры на нашей площадке одна таблица.
  // Своих таблиц на игру у Яндекса бывает несколько (очки, время, уровень), и
  // когда это понадобится здесь, имя станет частью адреса — пока лишняя
  // сущность только запутала бы.
  leaderboard: {
    async setScore(a, name, score) {
      // Не вошёл — рекорд отправлять некуда: таблица без имён это список
      // безымянных чисел. Молча ничего не делаем, игру этим не тревожим.
      ...
      try {
        await fetch('/api/score', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ game: gameKey(), score: value }),
        });
      } catch (e) {
        // Сеть подвела — не беда: рекорд это не прогресс, потерять его
        // не страшно, а ронять игру из-за таблицы тем более незачем.
      }
    },

    async getPlayerEntry() {
      try {
        const res = await fetch('/api/score?game=' + gameKey());
        if (!res.ok) return null;
        const data = await res.json();
        return data.mine ? { rank: data.mine.place, score: data.mine.score } : null;
      } catch (e) {
        return null;
      }
    },

    async getTop(a, name, n = 10) {
      try {
        const res = await fetch('/api/score?game=' + gameKey());
        if (!res.ok) return [];
        const data = await res.json();
        return (data.top || []).slice(0, n).map((r) => ({
          rank: r.place, score: r.score, name: r.name,
        }));
      } catch (e) {
        return [];
      }
