...
export const GAME_ID = 'cosmic-drift';
if (GAME_ID === 'game') {
...
// v1 -> v2: появился объект meta { crystals, bestScore, upgrades } — мета-прогресс.
//           Старый сейв (демо-шаблона с полем score) приводим к рабочему meta.
const MIGRATIONS = {
  1: (d) => {
    if (!d.meta) d.meta = { crystals: 0, bestScore: 0, upgrades: {} };
    if (typeof d.meta.crystals !== 'number') d.meta.crystals = 0;
    if (typeof d.meta.bestScore !== 'number') d.meta.bestScore = d.score || 0;
    if (!d.meta.upgrades) d.meta.upgrades = {};
    return d;
  }
};
...
export const Platform = {
  ...
  // Возвращаем null, а НЕ жёсткий 'ru', когда SDK недоступен: жёсткий дефолт здесь
  // проглатывал язык браузера — англоязычный игрок получал русский интерфейс (отказы
  // п.2.14/8.2.3: «Рубеж» 2026-07-29, «Осуши озеро» 2026-08-20; шаблон v35). Запасной
  // язык подставляет ВЫЗЫВАЮЩИЙ (main.js: ... || navigator.language), где цепочка полная.
  ...
  // ---- Сырое локальное хранилище --------------------------------------------
  // Та же безопасная площадка-осведомлённая обёртка, что и у зеркала меты ниже
  // (store()) — учитывает VK: сырой localStorage не переживает reload в их
  // sandbox-iframe (см. store()). Раньше store() была приватной; отдаём наружу
  // маленький срез для НЕ-мета данных с собственным жизненным циклом — например,
  // чекпоинт текущего забега (platform/runSave.js): ему не нужны ни облако, ни
  // дебаунс, ни версии/миграции Platform.save, но нужна та же VK-безопасность.
  local: {
    getItem(key) { try { return store()?.getItem(key) ?? null; } catch (e) { return null; } },
    setItem(key, value) { try { store()?.setItem(key, value); } catch (e) {} },
    removeItem(key) { try { store()?.removeItem(key); } catch (e) {} },
  },
};
