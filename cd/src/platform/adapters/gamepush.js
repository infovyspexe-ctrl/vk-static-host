...
export const GamePushAdapter = {
  ...
  async init() {
    ...
    this.gp = await new Promise((resolve) => {
      ...
      // Без этого медленный/висящий/заблокированный gamepush.com оставляет await навечно,
      // и игра не грузится — чёрный экран. onerror ловит только ОКОНЧАТЕЛЬНЫЙ провал
      // загрузки, а на таймауте он приходит через минуты. Свой предохранитель: не
      // дождались SDK за TIMEOUT — идём в локальный режим (а слой сверху — на Yandex-fallback).
      // 5 с: когда хост доступен, SDK поднимается за ~секунду, на живых игроков не влияет.
      let settled = false;
  ...
  ready() {
    // GamePush-аналог ready. НЕ полагаемся на его маппинг в LoadingAPI.ready Яндекса:
    // на модерации «Решала» 2026-08-03 консоль сообщила «игра ни разу не запускалась» —
    // gameStart() до Яндекса сигнал НЕ донёс. Прямой ready Яндексу теперь всегда шлёт
    // слой (index.js → signalYandexReady), независимо от этого вызова.
  ...
  ads: {
    ...
    // Прослойка про предзагрузку не сообщает — не мешаем кнопке награды рисоваться.
