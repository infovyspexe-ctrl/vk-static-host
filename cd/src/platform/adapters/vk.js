...
export const VkAdapter = {
  ...
  // ГРАБЛЯ №3, реальный отказ модерации (2026-07-29, «Кладовая»): «Прогресс сбрасывается
  // при обновлении страницы». Причина — ДВЕ ошибки разом:
  //
  // 1) Раньше здесь был отдельный щуп storageAvailable через VKWebAppStorageGet с
  //    таймаутом 2 с. В реальном VK-клиенте (не в локальном смоук-тесте, где
  ...
  ads: {
    ...
    // VK умеет отвечать честно — тем же check(), которым showRewarded защищается от показа
    // без инвентаря. Кнопку награды без предзагрузки рисовать нельзя (п.5.1.5.3).
    hasRewarded(a) { return this.check(a, 'reward'); },
