...
export function createButton(scene, x, y, label, onClick, opts = {}) {
  ...
  // Отступы можно ужать под тесную раскладку (пример: шесть вкладок магазина в 720 px —
  // при штатных 34 px кнопки налезали друг на друга, поймано живым скриншотом sweet-empire).
  const padX = opts.paddingX ?? THEME.button.paddingX;
  const padY = opts.paddingY ?? THEME.button.paddingY;
  ...
  // Смена цвета фона на лету — для кнопок с состояниями (вкладки: выбранная/невыбранная).
  // Гасить невыбранную прозрачностью нельзя: она начинает читаться как ОТКЛЮЧЁННАЯ, а весь
  // ряд выглядит тусклым (замечание владельца на sweet-empire, особенно заметно на телефоне).
  container.setBgColor = (c) => { color = c; redraw(); };
